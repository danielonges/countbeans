...
3. **Split mode** — *Equal* / *Exact* / *Percent* / *Weight* buttons.
4. **Per-person shares** — for any non-equal mode, each participant's share is typed via a `ForceReply`; *Confirm* appears only once the shares reconcile (percent = 100, exact = the amount, weighted = any positive total).
5. **Confirm** — writes the expense and shows the same receipt as the one-liner, with the same coverage-gap / payer-excluded / `#general` nudges and the `/void` hint.
