...
async def test_balance_empty_group(dispatcher, session: AsyncSession) -> None:
    ...
    bot = MockedBot()
    await feed(dispatcher, bot, make_message("/balance", from_id=1001, username="caller"), session=session)
    assert "no outstanding balances" in (bot.last_reply or "").lower()


async def test_balance_personal_shows_what_you_owe(dispatcher, session: AsyncSession) -> None:
...
async def test_balance_all_lists_members_and_transfer(dispatcher, session: AsyncSession) -> None:
    await _seed_caller_owes_bob(session)
    bot = MockedBot()
