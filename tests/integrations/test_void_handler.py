"""Handler tests for /void — scope-aware undo of the most recent expense.

Covers: a void clears the expense from derived balances; permission refusal for
a non-admin non-owner; nothing-to-void; and scope isolation (an active event's
void never touches a general expense, and vice versa).
"""
...
async def test_void_clears_expense_from_balances(
    dispatcher, session: AsyncSession
) -> None:
    """The most-recent general expense is voided and drops out of derived balances."""
    ...
    assert "Voided" in (bot.last_reply or "")
    assert "5.00" not in (bot.last_reply or "")  # echoes the 10.00 amount, not a share
    assert "10.00" in (bot.last_reply or "")
...
async def test_void_refused_for_non_owner_non_admin(
    ...
    can't void it — and nothing is written."""
    group = await seed_group(session)
    bob = await seed_member(session, group, telegram_user_id=2002, username="bob")
...
async def test_void_allowed_for_admin_non_owner(
    ...
    """A group admin may void an expense someone else recorded."""
    group = await seed_group(session)
    bob = await seed_member(session, group, telegram_user_id=2002, username="bob")
    await seed_member(session, group, telegram_user_id=1001, username="caller")
    await seed_expense(session, group, payer=bob, participants=[bob], amount_cents=1000)
...
async def test_void_in_event_leaves_general_expense(
    ...
    general expense — scope isolation."""
    group = await seed_group(session)
    caller = await seed_member(session, group, telegram_user_id=1001, username="caller")
    bob = await seed_member(session, group, telegram_user_id=2002, username="bob")
    ...
    reply = bot.last_reply or ""
    assert 'in "Bali"' in reply  # scope note proves it hit the event scope
...
async def test_void_general_leaves_event_expense(
    ...
    assert '"Bali"' not in reply  # general scope has no event note
    assert "10.00" in reply  # the general expense, not the more-recent event one
