...
- **Service core** (`src/countbeans/services/`, the "Expense manager"): the **only** place that issues SQL against the database (the `UnitOfWork` and repositories live here — see "Database sessions" below). Validates and records expenses/settlements as ledger events, computes derived balances, runs `simplify()`. **Stateless** (no per-conversation state) and **transactional** (one SQLAlchemy transaction per command). Accepts and returns plain **Pydantic DTOs** — never `aiogram` or HTTP request/response types — so it has no knowledge of who called it. **Error contract:** a domain-rule violation whose message is safe to show the user raises `DomainError` (`services/errors.py`, a `ValueError` subclass) — handlers catch exactly that (plus pydantic `ValidationError` at DTO construction, humanized via `humanize_validation_error`); any other exception is a bug and propagates to the transactional middleware (rollback + log), never into a chat reply.
- **Bot layer** (`src/countbeans/bot/` — `server.py` + `handlers/` + `middleware/`, `aiogram`): a thin Telegram adapter and the **only runtime entry point today**. Parses command grammar (amount, description, participants), makes Telegram-only calls (`getChatMember`, `getChatMemberCount`), calls the service core in-process, and formats replies. Owns nothing the service core owns. Cross-cutting concerns live in middleware (see "Bot runtime & middleware" below): a per-update `UnitOfWork`, request-scoped logging context, and an admin gate that refuses group commands until the bot itself is an administrator. The per-command *caller*-admin checks for `/simplify` and `/currency` are separate inline `getChatMember` checks (`bot/utils/permissions.py`). Every group command opens through `bot/utils/context.py`'s `resolve_chat_context` (the shared group-upsert → caller-upsert → membership → active-event sequence; the `ChatContext` it returns also derives the effective scope — see "Database sessions" below). aiogram's FSM (backed by `MemoryStorage`) powers the **interactive `/addexpense` wizard**: a bare `/addexpense` (no args) launches a guided, button-driven flow (the `bot/handlers/addexpense_wizard/` package — `states` holds the FSM states + typed `WizardDraft`, `render` the anchor/prompt painting, `steps` the ForceReply free-text steps, `actions` the inline-button callbacks + submit) that collects the same fields and calls the same `add_expense` path, while the one-liner grammar is unchanged. The `@all` coverage check remains a non-blocking warning, not a confirm step.
...
**Bot runtime & middleware (`bot/server.py`).** `run()` is the composition root: it builds the `Dispatcher` (with `MemoryStorage` backing the `/addexpense` wizard's FSM — in-memory is right for this single-instance long-poll bot; a half-finished wizard is simply abandoned on restart), registers middleware, includes the handler routers, publishes the command menus, and starts long-polling. Three middlewares wrap every update, and **their registration order is load-bearing**:
...
- **The service core defines a `UnitOfWork`** — an async context manager that wraps the `async_sessionmaker`, opens one `AsyncSession` + transaction, exposes the repositories (`uow.expenses`, `uow.users`, …), and **commits on clean exit / rolls back on exception**. It is the *only* type holding SQLAlchemy session machinery — callers never import `AsyncSession`/`select`. One documented exception on the read side: repositories return the **identity/context ORM rows** (`Group`, `User`, `Event`), which flow to handlers as read-only context — bundled per command by `bot/utils/context.py`'s `ChatContext` (built via `resolve_chat_context`, which owns the group-upsert → caller-upsert → membership → active-event opening sequence shared by the group commands). All mutations still go through commands + repository methods, and money-bearing reads remain DTO-only.
...
- **No `from_attributes=True`.** DTOs are **not** built directly from ORM rows via `model_validate(row)` — that would tie DTO field names to ORM attribute names. Instead, each repository method maps rows to DTOs explicitly (`_to_dto(row) -> SomeResult`); the ledger write methods (`settlements.add`, `expenses.add`, `events.create`) take plain fields, construct the ORM row (and its UUID7 id) internally, and return the result DTO or id — so service modules import no ORM models. The ORM schema can evolve without the DTO shape changing, and the boundary stays airtight.
