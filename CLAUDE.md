...
- **Bot layer** (`src/countbeans/main.py` + handlers, `aiogram`): a thin Telegram adapter and the **only runtime entry point today**. Parses command grammar (amount, description, participants), holds **any multi-step conversational state in aiogram's FSM** (none is load-bearing today — the `@all` coverage check is a non-blocking warning, and the `/simplify`/`/currency` admin gates are inline `getChatMember` checks), makes Telegram-only calls (`getChatMember`, `getChatMemberCount`), calls the service core in-process, and formats replies. Owns nothing the service core owns.
